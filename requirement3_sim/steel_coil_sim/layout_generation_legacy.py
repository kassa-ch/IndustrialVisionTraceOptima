...
# 钢卷统一尺寸（单位：m）
# 这里把钢卷建模为“轴向沿 A 的中空圆柱”：
# - A 方向是钢卷轴向长度
# - B/Z 方向对应钢卷端面的外径
# - 顶视图（AB 平面）仍然表现为一个矩形投影，这也是路径规划实际使用的包络
COIL_AXIS_LENGTH_A = 0.50
COIL_OUTER_DIAMETER_M = 0.80
COIL_INNER_DIAMETER_M = 0.24
COIL_OUTER_RADIUS_M = COIL_OUTER_DIAMETER_M / 2.0
COIL_INNER_RADIUS_M = COIL_INNER_DIAMETER_M / 2.0

# 兼容下游脚本使用的旧字段命名
COIL_WIDTH_A = COIL_AXIS_LENGTH_A
COIL_LENGTH_B = COIL_OUTER_DIAMETER_M
COIL_HEIGHT_Z = COIL_OUTER_DIAMETER_M

# 底层相邻钢卷沿B方向间隙：2cm~8cm
GAP_MIN = 0.02
GAP_MAX = 0.08

# 列整体在B方向的上下偏移（用于打破各列完全齐平）
COLUMN_B_SHIFT_MIN = -0.12   # -12cm
COLUMN_B_SHIFT_MAX = 0.12    # +12cm
...
def generate_column_b_positions_with_shift(
    n: int,
    column_b_shift: float
) -> List[Tuple[float, float]]:
    """
    为单独一列生成下层钢卷的 B 边界，并施加整列上下偏移。
    这样：
    1. 每列自己的相邻钢卷间距独立随机
    2. 每列整体在B方向有上下错动，不会与其他列完全对齐
    """
    positions = generate_base_column_b_positions(n)
    shifted_positions = []
    for b_bottom, b_top in positions:
        shifted_positions.append((b_bottom + column_b_shift, b_top + column_b_shift))
    return shifted_positions

def build_base_column_rectangles(
    base_a: float,
    b_positions: List[Tuple[float, float]],
    column_b_shift: float = 0.0
) -> List[Dict]:
    """
    生成某一列下层钢卷
    """
    ...
    for i, (b_bottom, b_top) in enumerate(b_positions, start=1):
        ...
        coils.append({
            ...
            "z_top": z_top,
            "a_offset": a_offset,
            "b_shift": column_b_shift,
            "orientation": "轴向沿A，环形端面位于左右两侧",
            "axis_length_a": COIL_AXIS_LENGTH_A,
            "outer_diameter": COIL_OUTER_DIAMETER_M,
            "inner_diameter": COIL_INNER_DIAMETER_M,
            "outer_radius": COIL_OUTER_RADIUS_M,
            "inner_radius": COIL_INNER_RADIUS_M,
        })
    return coils
...
def generate_coils_layout() -> List[Dict]:
    ...
    修改点：
    1. 每一列独立生成自己的 B 方向位置，不再复用同一套 base_b_positions
    2. 每一列再施加一个整列级别的 B 方向上下偏移，避免各列完全齐平
    """
    ...
    for col_idx, base_a in enumerate(base_as, start=1):
        # 每一列单独生成“整列上下偏移”
        column_b_shift = random.uniform(COLUMN_B_SHIFT_MIN, COLUMN_B_SHIFT_MAX)

        # 每一列单独生成自己的下层B位置
        b_positions = generate_column_b_positions_with_shift(
            BASE_COILS_PER_COLUMN,
            column_b_shift
        )

        base_coils = build_base_column_rectangles(base_a, b_positions, column_b_shift)
...
def plot_scene(
    ...
    # 画钢卷顶视投影。先画下层，再画上层，保证第二层在视觉上覆盖第一层。
    sorted_coils = sorted(
        coils,
        key=lambda item: (
            item["layer"],
            item["z_bottom"],
            0.5 * (item["b_bottom"] + item["b_top"]),
            item["a_left"],
        ),
    )
    for coil in sorted_coils:
        ...
        if use_layer_colors:
            face_color = "#bdbdbd" if coil["layer"] == 1 else "#90b4ff"
        ...
        patch = Polygon(
            ...
            alpha=0.96 if opaque else 0.28,
    ...
    inset = ax.inset_axes([0.74, 0.03, 0.22, 0.22])
    outer_face = Circle((0.0, 0.0), COIL_OUTER_RADIUS_M, facecolor="#bdbdbd", edgecolor="black", linewidth=1.2, alpha=0.85)
    inner_hole = Circle((0.0, 0.0), COIL_INNER_RADIUS_M, facecolor="white", edgecolor="black", linewidth=1.0)
    inset.add_patch(outer_face)
    inset.add_patch(inner_hole)
    inset.set_title("钢卷端面环形示意", fontsize=8)
    inset.text(
        0.0,
        -COIL_OUTER_RADIUS_M - 0.06,
        f"外径={COIL_OUTER_DIAMETER_M:.2f}m\n内径={COIL_INNER_DIAMETER_M:.2f}m",
        ha="center",
        va="top",
        fontsize=7,
    )
    inset.set_aspect("equal", adjustable="box")
    inset.set_xlim(-COIL_OUTER_RADIUS_M - 0.08, COIL_OUTER_RADIUS_M + 0.08)
    inset.set_ylim(-COIL_OUTER_RADIUS_M - 0.22, COIL_OUTER_RADIUS_M + 0.08)
    inset.axis("off")

    # 图例
    legend_elements = [Line2D([0], [0], color="gray", lw=2.0, label="场地边界")]
    if use_layer_colors:
        legend_elements = [
            Line2D([0], [0], color="black", lw=1.5, label="下层钢卷顶视投影"),
            Line2D([0], [0], color="blue", lw=1.5, label="上层钢卷顶视投影"),
            *legend_elements,
        ]
    else:
        legend_elements = [
            Line2D([0], [0], color="#303030", lw=1.5, label="钢卷顶视投影（统一灰阶）"),
            *legend_elements,
        ]
    ax.legend(handles=legend_elements, loc="upper center", bbox_to_anchor=(0.5, 0.98), frameon=True, ncol=2)

    ax.set_xlabel("A coordinate (m)")
    ax.set_ylabel("B coordinate (m)")
    style_suffix = "Layer-colored" if use_layer_colors else "Monochrome"
    title_suffix = "Solid Top View" if opaque else "Schematic Top View"
    ax.set_title(f"Scene {scene_id}: Annular Coil {style_suffix} {title_suffix} in AB Coordinates")
    ...
    for scene_id in range(1, NUM_SCENES + 1):
        ...
        for coil in coils:
            ...
            csv_rows.append({
                ...
                "钢卷轴向长度A(cm)": round(coil["axis_length_a"] * 100.0, 2),
                "钢卷外径(cm)": round(coil["outer_diameter"] * 100.0, 2),
                "钢卷内径(cm)": round(coil["inner_diameter"] * 100.0, 2),
                "钢卷外半径(cm)": round(coil["outer_radius"] * 100.0, 2),
                "钢卷内半径(cm)": round(coil["inner_radius"] * 100.0, 2),
        ...
        fig_schematic_path = os.path.join(OUTPUT_DIR, f"scene_{scene_id}_schematic.png")
        fig_gray_path = os.path.join(OUTPUT_DIR, f"scene_{scene_id}_gray.png")
        fig_gray_schematic_path = os.path.join(OUTPUT_DIR, f"scene_{scene_id}_gray_schematic.png")
        plot_scene(scene_id, site, coils, fig_path, opaque=True, use_layer_colors=True)
        plot_scene(scene_id, site, coils, fig_schematic_path, opaque=False, use_layer_colors=True)
        plot_scene(scene_id, site, coils, fig_gray_path, opaque=True, use_layer_colors=False)
        plot_scene(scene_id, site, coils, fig_gray_schematic_path, opaque=False, use_layer_colors=False)
